"""Import a plain local folder (no Git)."""
...
from codebase_architect.infrastructure.source_providers._common import copy_tree
from codebase_architect.shared.ids import new_id


class LocalFolderSourceProvider(SourceProvider):
    """Materializes a local directory that is not a Git repository."""

    source_type: ClassVar[SourceType] = SourceType.FOLDER

    def supports(self, location: SourceLocation) -> bool:
        path = Path(location.raw)
        return path.is_dir() and not (path / ".git").exists()
    ...
    def fetch(self, location: SourceLocation, dest: Path) -> Workspace:
        ...
        checksum = compute_tree_checksum(dest, DEFAULT_IGNORED_DIRS)
        return Workspace(
            id=new_id("ws"),
            root_path=dest,
